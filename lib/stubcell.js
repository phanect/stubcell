...
StubCell.prototype._getMatchEntry = function(req, entries){
  entries = entries.filter(function(entry) {
    return check.api(entry.api, req, this.looseCompare);
  });
  ...
  switch (specs.length) {
    ...
    case 0:
      return null;
    default: // 2 or more
      var msg = "Duplicate specs in " + entry.api.url + " (" + entry.api.method + ")";
      console.error(colo.red(msg));
      throw new Error(msg);
  }
};

/**
 * Return spec matched to sent query
 * @param {Object} req Express request object
 * @param {entry} entry Entry matched to request URL
 */
StubCell.prototype._getMatchSpec = function(req, entry) {
  var specs = entry.api.specs.filter(function(spec, i) {
    if ((spec.body||{}).jsonrpc) {
      if (typeof req.body.id !== "undefined") spec.body.id = req.body.id;
    }

    return check.spec(spec, req, this.looseCompare);
  });
  ...
  switch (specs.length) {
    ...
    default: // 2 or more
      var msg = "Duplicate specs in " + entry.api.url + " (" + entry.api.method + "):";
        + specs.map(function(spec) { return spec.name }).join(" and ");
      console.error(colo.red(msg));
      throw new Error(msg);
  }
};
...
StubCell.prototype._validateEntry = function(){
  ...
  return function(entry){
    if(methods.indexOf(entry.api.method.toLowerCase()) === -1){
...
StubCell.prototype._execEntry = function(req, res, entry, spec){
  var entryUrl  = entry.api.url;
  var response  = spec.response;
  var headers   = response.headers || {};
  var entryBody = spec.request.body || {};
