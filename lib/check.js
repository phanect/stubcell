...
var check = module.exports = {
  ...
  api: function(api, req, looseCompare){
    var parsedUrl = url.parse(req.url);
    return api.method === "ALL" ? true : api.method.toLowerCase() === req.method.toLowerCase() &&
      check.url(api.pathRegexp, parsedUrl.pathname);
  },

  spec: function(spec, req, looseCompare){
    return check.headers(spec.headers, req.headers, looseCompare) &&
      check.query(spec.query, req.query, looseCompare) &&
      check.body(spec.body, req.body, looseCompare);
  },
